...
import { useGetRewardsByDelegator } from "../../../hooks/useGetRewardsByDelegator";
...
const RewardsByValidatorsList = () => {
  ...
  const queryRewards = useGetRewardsByDelegator(
    id?.toString() || "",
    Number(page)
  );
  ...
  const headers = ["Era", "Date", "Amount"];
  const rows = queryRewards.data?.data.map(item => {
    return [
...
export default RewardsByValidatorsList;
